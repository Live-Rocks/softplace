...
import type { Memory } from "@softplace/shared";
...
export function MemoriesScreen({ accessToken, embedded = false, showHeader = true }: Props) {
  ...
  async function saveMemory() {
    ...
    try {
      if (editingId) {
        await api.updateMemory(editingId, { content: trimmed }, accessToken);
      } else {
        await api.createMemory({ content: trimmed, category: "preference" }, accessToken);
      }
      setContent("");
      setEditingId(null);
      await load();
