...
test("generation context injects at most two threshold-passing chunks and never includes assistant text", () => {
  const candidates = [
    ...
    candidate("c3", 3, 0.64, [message("u3", 7, "user", "第三段")]),
    candidate("c4", 4, 0.59, [message("u4", 9, "user", "低於門檻")])
  ];
  const prepared = prepareGenerationContext(candidates);
  assert.ok(prepared);
  assert.deepEqual(prepared.injectedChunkIds, ["c1", "c2"]);
  assert.match(prepared.text, /第一次被改企劃|那次也很慌|後來同事來幫忙/);
  assert.doesNotMatch(prepared.text, /舊助理推論|另一段舊回答|第三段|低於門檻/);
  assert.equal((prepared.text.match(/那次也很慌/g) ?? []).length, 1);
});

test("generation context abstains when top one misses threshold and respects the 1200-token budget", () => {
  assert.equal(prepareGenerationContext([
    candidate("c1", 1, 0.59, [message("u1", 1, "user", "不注入")]),
    candidate("c2", 2, 0.58, [message("u2", 2, "user", "也不注入")])
  ]), null);
  const prepared = prepareGenerationContext([
    candidate("large", 1, 0.9, [message("large-u", 1, "user", "繁體中文內容".repeat(1000))])
  ]);
  assert.ok(prepared);
...
test("generation report requires 25 reviewed injected runs, half helpful, and zero harm", () => {
  ...
  assert.equal(report.phase2Pass, true);
  ...
  assert.equal(buildGenerationReport(harmful, candidates).phase2Pass, false);
});
